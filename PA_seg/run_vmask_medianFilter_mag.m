function run_vmask_medianFilter_mag(alpha_thr)
mid=get_mid_number;
q=get_quality;
...
for i=6
    for j=1:2
        savename=sprintf('PVS%02d/vmask_Mag_PVS%02d_MID%d.mat',i,i,mid(i,j+1));
        if exist(savename,'file')
            continue;
        end
        fname=sprintf('../mat_phase_mag/PVS%02d_mat/Mag_PVS%02d_MID%d.mat',i,i,mid(i,j+1));
        if ~exist(fname,'file')
            continue;
        end
        ...
        params.ispc=false;
        wm_mask=ri_d1(fname);
        params.wm_mask=clusterize2(wm_mask,100);% remove isolated voxels
