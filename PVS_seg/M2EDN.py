...
def fmeasure_loss_new(y_true, y_pred):

    
    vfn = fn(y_true, y_pred)
    vfp = fp(y_true, y_pred)
    alpha=5; #relative weight for fn
    cost=(vfn+vfp/alpha)/(168*168*168*0.005)  
    return cost
...
def get_m2edn(numoflandmarks,inputchannel,imagesize):
    ...
    model.compile(optimizer='Adam', 
                  metrics=[dsc,fp,fn,npvs], 
                  loss=fmeasure_loss_new)
