...
def Generator_MultiChannel_Test_Chunks_cl(pathofchannels, idx_subject, sizeofchunk, 
                                          sizeofchunk_expand, numofchannel, ave_im=0, std_im=1.0):
    
    inputs = []
    for i_channel in range(numofchannel):
        if i_channel == 0:
            image = np.array(sitk.GetArrayFromImage(sitk.ReadImage(pathofchannels[i_channel]+idx_subject)))
