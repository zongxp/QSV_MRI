function Fit_MAP(params,save_name)
...
for i=1:nroi
    ...
    [data(:,:,i),roi,data_debug(i)]=calc_ph(params,roi_vessel,i);
    ...
    fit_par.roi=roi;
    fit_par.sz=size(roi);
    fitfunc2 = @(x) MAP_fitfunc(x,fit_par);
    ...
    [resid,ph_fit(:,:,i)]=fitfunc2(res);
    
    if isfield(params,'plot_fit') && params.plot_fit  %check fit
        plot_fit(data(:,:,i),ph_fit(:,:,i),roi);
    end
...
save(save_name,'v','rad','ph_fit','data','center','params','exitflag','output','roi_vessel','data_debug');
...
cd_fit(~roi)=0;
figure; plot(data,cd_fit,'o');
hold on;plot(min_max(data),min_max(data),'r-');
resid=sos(data(:)-cd_fit(:),1).^2;
disp(resid);

function [ph,roi3,data_debug]=calc_ph(p,roi_vessel,i)
...
roi3=mask_circle(size(ph),roiRad_i,pos,1);
